...
function App() {
  ...
  return (
    <>
      <div className="relative h-screen bg-[url('src/assets/img/minions_bg_light.jpg')] bg-no-repeat bg-cover">
        <header className="h-36 pl-12">
          <img src="src/assets/img/minions_logo.png" className="h-full" />
          <div>
            <p>{score.current}</p>
        ...
        <div className="max-w-screen-xl w-fit mx-auto grid grid-cols-3 gap-10">
          {minions.map((minion) => (
            <Card
              name={minion.name}
              imgUrl={`src/assets/img/minions/minion-${minion.name}.webp`}
              flip={flip}
              key={minion.name}
